TARGETED 227-PERSON REDO OVERRIDE

This customized copy changes only the orchestration target. All other instructions below remain authoritative except where this block explicitly overrides global completion language.

- Set `TARGET_MANIFEST=age-27-people/backups/2026-08-27-enrichment-redo-227/target-manifest.json`.
- Add `--target-manifest "$TARGET_MANIFEST"` to every helper `status` and `select` command. Include `TARGET_MANIFEST` in every normal and recovery worker handoff and require the worker to pass it to `select`.
- Process only QIDs in that frozen manifest. Continue serially with `BATCH_SIZE=100`; manifest-aware selection must produce exactly 100, 100, and 27 people.
- Use `target_remaining`, not the global `eligible_count`, as the progress and completion authority. Stop as soon as `target_remaining` is zero even though other globally eligible rows remain.
- Never spawn a fourth normal cohort and never select, enrich, or modify a person outside the manifest. Treat any out-of-manifest selection or cohort-size sequence other than 100/100/27 as a blocker.
- The final report must give the 227-person target count, the three cohort sizes, target remaining count, and every `possible_removal` case and reason.
